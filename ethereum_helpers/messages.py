...
def sign_message(message: str,
                 signing_key_string: str) -> str:
    message_hash = hash_message(message)
    v, r, s = signature_triplet(message_hash=message_hash,
                                signing_key_string=signing_key_string)
    return hex(r) + hex(s)[2:] + hex(v)[2:]
...
def signature_triplet(
    ...
    signing_key_int = hex_string_to_int(signing_key_string)
...
def deterministic_generate_k(*,
                             message_hash: str,
                             signing_key_string: str) -> int:
    v = b'\x01' * 32
    k = b'\x00' * 32
    signing_key_bytes = encode_number(hex_string_to_int(signing_key_string),
                                      base=256,
                                      min_length=32)
    message_hash_bytes = encode_number(hex_string_to_int(message_hash),
                                       base=256,
                                       min_length=32)
    k = hmac.new(k,
